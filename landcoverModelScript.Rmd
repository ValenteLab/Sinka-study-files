...
rm(lowerLandsatData, upperLandsatData, mosaickedLandsat)
...
# Stack the original bands and the indices together
...
Reclassifying NLCD and TCC

```{r}
# Creating a reclassified raster focusing on the "Value" attribute for tcc and nlcd
tcc <- classify(tcc_utm, rcl = matrix(c(0, 100, 1), ncol = 2, byrow = TRUE)) 

# Defining reclassification matrix based on NLCD "Value"
rcl_matrix <- matrix(c(
  0, 11, 11,  # Open Water
...
nlcd_reclassified = as.factor(nlcd_reclassified)
...
nlcd_reclassified = resample(nlcd_reclassified, tcc, method='near')
...
I UPDATED THE CODE BELOW BY ELIMINATING THE SECTION THAT ACTUALLY NORMALIZED THE PREDICTORS. PLEASE CLEAN THIS UP AND PROVIDE A NEW VERSION OF THE CODE.
...
#save(rf_model, file = "C:/Users/sza0209/OneDrive - Auburn University/Documents/MSc/Thesis/Study #1/rf_model2.RData")
```
...
writeRaster(map, 'jvClassificationModel.tif')
